...
const cleanupDeployWorkflow = (deployWorkflow, deployWorkflowPath) => {
	deployWorkflow.jobs.typecheck = undefined;
...
const cleanupVitestConfig = (vitestConfig, vitestConfigPath) => {
	const newVitestConfig = vitestConfig.replace('setup-test-env.ts', 'setup-test-env.js');
...
const main = async ({ isTypeScript, packageManager, rootDirectory }) => {
	...
	const CREATE_USER_COMMAND_PATH = path.join(CYPRESS_SUPPORT_PATH, `create-user.${FILE_EXTENSION}`);
	const DELETE_USER_COMMAND_PATH = path.join(CYPRESS_SUPPORT_PATH, `delete-user.${FILE_EXTENSION}`);
	...
	if (!isTypeScript) {
		fileOperationPromises.push(...cleanupDeployWorkflow(deployWorkflow, DEPLOY_WORKFLOW_PATH));

		fileOperationPromises.push(...cleanupVitestConfig(vitestConfig, VITEST_CONFIG_PATH));
	}
